...
def ml_explain_tab(df):
    ...
    # Extract datetime components & drop original datetime columns
    datetime_cols = x_df.select_dtypes(include=['datetime', 'datetime64[ns]', 'datetime64']).columns
    for col in datetime_cols:
        x_df[f"{col}_year"] = x_df[col].dt.year
        x_df[f"{col}_month"] = x_df[col].dt.month
        x_df[f"{col}_day"] = x_df[col].dt.day
        x_df[f"{col}_weekday"] = x_df[col].dt.weekday
    x_df = x_df.drop(columns=datetime_cols)

    # Remove any remaining object columns with un-encodable types
    x_df = pd.get_dummies(x_df, drop_first=True)

    # Align X/y for rows not null in target
    mask = y.notnull()
    x_df, y = x_df[mask], y[mask]

    # Fix for deprecated is_categorical_dtype
    clf_task = pd.api.types.is_object_dtype(y) or isinstance(y.dtype, pd.CategoricalDtype)
